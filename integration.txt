...
const registerImageMeta = useCallback((uri: string, w: number, h: number) => {
  ...
  useFocusEffect(
    useCallback(() => {
      console.log('🔄 [useFocusEffect] Screen focused - reloading photos');
      let isMounted = true;
      ...
      return () => {
        isMounted = false;
        clearTimeout(timer);
        console.log('🔄 [useFocusEffect] Screen unfocused - cleanup');
      };
    }, [loadPhotoItems])
  );
  ...
  // --- Sistema multi-touch: no necesitamos handleDeletePhoto antiguo ---
    ...
    return memo(({
      ...
      return (
        <Canvas style={[styles.canvas, { height: canvasHeight }]}> 
          ...
          {Children.toArray(telePaths.map((pathInfo: any) => (
            <Path
              key={`t-${pathInfo.idx}`}
              path={pathInfo.path}
          ...
          {Children.toArray(highlightPaths.map((pathInfo: any) => (
