...
const OptimizedWhiteboardDebug = ({ 
  ...
  // Console.log de las props recibidas
  console.log('=== PROPS RECIBIDAS EN WHITEBOARD ===');
  console.log('Rate General ID:', rateGeneralId);
  console.log('Table ID:', tableId);
  console.log('Stick Bonus inicial:', stickBonus);
  console.log('Percentage:', percentage);
  console.log('Callbacks:', {
    setStickBonusset: typeof setStickBonusset,
    oncodetable: typeof oncodetable,
    onSaveComplete: typeof onSaveComplete
  });
  ...
  useEffect(() => {
    ...
    Animated.parallel([
      ...
      // Bring in eraser button
      Animated.timing(eraserButtonAnim, {
        toValue: 190, // Position it to the right of the redo button
        duration: 300,
        useNativeDriver: true,
        easing: Easing.out(Easing.back(1.5))
      }),
      ...
      Animated.timing(penButtonAnim, {
        toValue: 250, // Position it to the right of the eraser button
        duration: 300,
        useNativeDriver: true,
        easing: Easing.out(Easing.back(1.5))
      }),
  ...
  const loadDrawingData = async () => {
    try {
      ...
      if (tableId) {
        ...
        // Console.log completo de datos de la competición
        console.log('=== DATOS COMPLETOS DE LA COMPETICIÓN ===');
        console.log('Table ID:', tableId);
        console.log('Rate General completo:', rateGeneral);
        
        if (rateGeneral) {
          console.log('--- INFORMACIÓN DETALLADA DE LA COMPETICIÓN ---');
          console.log('ID del registro:', rateGeneral.id);
          console.log('ID de la tabla:', rateGeneral.tableId);
          console.log('Stick Bonus activo:', rateGeneral.stickBonus);
          console.log('Número de elementos:', rateGeneral.numberOfElements);
          console.log('Valores de dificultad:', rateGeneral.difficultyValues);
          console.log('Grupos de elementos:', {
            grupo1: rateGeneral.elementGroups1,
            grupo2: rateGeneral.elementGroups2,
            grupo3: rateGeneral.elementGroups3,
            grupo4: rateGeneral.elementGroups4,
            grupo5: rateGeneral.elementGroups5
          });
          console.log('Puntuaciones de ejecución:', {
            execution: rateGeneral.execution,
            eScore: rateGeneral.eScore,
            myScore: rateGeneral.myScore
          });
          console.log('Puntuaciones de competición:', {
            compD: rateGeneral.compD,
            compE: rateGeneral.compE,
            compSd: rateGeneral.compSd,
            compNd: rateGeneral.compNd,
            compScore: rateGeneral.compScore
          });
          console.log('Comentarios:', rateGeneral.comments);
          
          if (rateGeneral.paths) {
            try {
              // Parse the stored paths data
              const parsedPaths = JSON.parse(rateGeneral.paths);
              console.log('Paths JSON parseado:', parsedPaths);
              console.log('Número de trazos guardados:', parsedPaths.length);
              ...
              if (Array.isArray(parsedPaths)) {
                ...
                setPaths(fixedPaths);
                console.log('Paths cargados en el estado:', fixedPaths.length, 'trazos');
                
                // Log de tipos de paths
                const pathTypes = {
                  normal: fixedPaths.filter(p => p.penType === 0).length,
                  telestrator: fixedPaths.filter(p => p.penType === 1).length,
                  highlighter: fixedPaths.filter(p => p.penType === 2).length,
                  eraser: fixedPaths.filter(p => p.isEraser).length
                };
                console.log('Distribución de tipos de trazos:', pathTypes);
              }
              
              // Also load stick bonus state
              if (typeof rateGeneral.stickBonus === 'boolean') {
                setStickBonusset(rateGeneral.stickBonus);
                console.log('Stick bonus configurado desde BD:', rateGeneral.stickBonus);
              }
            } catch (parseError) {
            ...
            console.log('No hay paths guardados para esta competición');
            setPaths([]);
          }
        } else {
          console.log('No se encontraron datos para el tableId:', tableId);
        }
      } else {
        console.log('No se proporcionó tableId para cargar datos');
      }
    } catch (error) {
    ...
    try {
      ...
      if (tableId) {
        ...
        if (rateGeneral) {
          ...
          // Console.log de los datos que se van a guardar
          console.log('=== GUARDANDO DATOS DE LA COMPETICIÓN ===');
          console.log('Table ID:', tableId);
          console.log('Rate General ID:', rateGeneral.id);
          console.log('Paths actuales a guardar:', currentPaths);
          console.log('Número de trazos:', currentPaths.length);
          console.log('Stick bonus actual:', stickBonus);
          console.log('Datos completos antes de guardar:', {
            ...rateGeneral,
            stickBonus: stickBonus,
            pathsCount: currentPaths.length
          });
          ...
          console.log('Datos actualizados para la BD:', updatedRateGeneral);
  ...
  useEffect(() => {
    if (!loading) {
      console.log('=== CAMBIO EN PATHS ===');
      console.log('Número de paths:', paths.length);
      console.log('Stick bonus actual:', stickBonus);
      console.log('Paths por tipo:', {
        normal: paths.filter(p => p.penType === 0).length,
        telestrator: paths.filter(p => p.penType === 1).length,
        highlighter: paths.filter(p => p.penType === 2).length,
        eraser: paths.filter(p => p.isEraser).length
      });
      debouncedSave();
    }
  }, [paths, stickBonus]);
  
  // Hook to save on component unmount
  useEffect(() => {
    return () => {
  ...
  const handleUndo = () => {
    if (paths.length > 0) {
      ...
      console.log('Paths antes del undo:', paths.length);
      console.log('Path que se va a deshacer:', paths[paths.length - 1]);
      ...
      setPaths(prev => {
        ...
        console.log('Paths después del undo:', newPaths.length);
  ...
  // Load persistent settings from AsyncStorage
  const loadPersistentSettings = async () => {
    try {
      const settingsString = await AsyncStorage.getItem(WHITEBOARD_SETTINGS_KEY);
      if (settingsString) {
        const settings = JSON.parse(settingsString);
        setSelectedColor(settings.selectedColor || DEFAULT_SETTINGS.selectedColor);
        setSelectedStrokeWidth(settings.selectedStrokeWidth || DEFAULT_SETTINGS.selectedStrokeWidth);
        setSelectedPen(settings.selectedPen || DEFAULT_SETTINGS.selectedPen);
        setIsEraser(settings.isEraser || DEFAULT_SETTINGS.isEraser);
        console.log("Loaded persistent whiteboard settings:", settings);
      } else {
        console.log("No persistent settings found, using defaults");
      }
    } catch (error) {
      console.error("Error loading persistent whiteboard settings:", error);
    }
  };
  ...
  const savePersistentSettings = async () => {
    try {
      ...
      console.log("Saved persistent whiteboard settings:", settings);
    } catch (error) {
      console.error("Error saving persistent whiteboard settings:", error);
    }
  };
  ...
  }, [selectedColor, selectedStrokeWidth, selectedPen, isEraser]);
  ...
  const clearCanvas = () => {
    ...
    // Save the empty canvas immediately
    setTimeout(() => saveDrawingData(), 100);
  };
  // Toggle stick bonus
  const toggleStickBonus = () => {
    const newStickBonus = !stickBonus;
    console.log('=== CAMBIO STICK BONUS ===');
    console.log('Anterior:', stickBonus);
    console.log('Nuevo:', newStickBonus);
    
    // Call the external setStickBonus function to update parent component
    setStickBonusset(newStickBonus);
    // Save the updated stick bonus immediately
    setTimeout(() => saveDrawingData(), 100);
  };
  ...
  const panGesture = Gesture.Pan()
    ...
    .onStart((event) => {
      ...
      // Skip if touching the eraser button area
      if (event.x > 180 && event.x < 240 && event.y < 60) {
        return;
      }
      
      // Skip if touching the pen button area
      if (event.x > 240 && event.x < 300 && event.y < 60) {
        return;
      }
      ...
      // On Android, always allow drawing (do not validate pen)
      ...
      // Determine if it's a stylus (for non-Android)
      const isPencil = 
      ...
      // Only proceed with drawing if it's a pen (for non-Android)
      if (isPencil) {
    ...
    <GestureHandlerRootView style={styles.container}>
      ...
      <GestureDetector gesture={panGesture}>
        <View        
          ...
          onMouseDown={Platform.OS === 'web' ? handleWebTouchStart : undefined}
          onMouseMove={Platform.OS === 'web' ? handleWebTouchMove : undefined}
          onMouseUp={Platform.OS === 'web' ? handleWebTouchEnd : undefined}
        >
          <Svg height="100%" width="100%">
            ...
            {paths
              ...
              .map((pathData, index) => (
                <Path
                  ...
                  strokeWidth={pathData.isEraser ? pathData.strokeWidth * 4 : pathData.strokeWidth}
                  fill="none"
                  strokeLinecap="round"
            ...
            {paths
              ...
              .map((pathData, index) => (
                <Path
                  ...
                  strokeWidth={pathData.isEraser ? 8 : 2}
                  fill="none"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  opacity={0.8}
              ...
              .map((pathData, index) => {
                ...
                return (
                  <Path
                    ...
                    strokeWidth={pathData.isEraser ? 4 : 1}
            ...
            {currentPath ? (
              <Path
                ...
                strokeWidth={isEraser ? (selectedPen === 1 ? 8 : selectedStrokeWidth * 4) : (selectedPen === 1 ? 2 : selectedStrokeWidth)}
                ...
                <View style={[
                  ...
                  {COLORS.map(color => (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        // When selecting a color, first switch to normal pen and turn off eraser
                        setSelectedPen(0);
