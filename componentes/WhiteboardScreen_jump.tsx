...
const WhiteboardScreen = memo(forwardRef<WhiteboardRef, WhiteboardProps>(({
  ...
  // Algoritmo Ramer-Douglas-Peucker optimizado
  const simplifyPoints = useCallback((points: Point[], tolerance: number = SIMPLIFY_TOLERANCE): Point[] => {
  ...
  const handleTouchEnd = useCallback(() => {
    if (currentPointsRef.current.length > 1) {
      ...
      console.log('✏️ Path finished:', { pathColor, capturedIsEraser, capturedSelectedPen, capturedColor, isHighlighter });
  ...
  const openVaultTable = useCallback(() => {
    console.log('📦 Opening vault table...');
    console.log('📦 Props:', { hasOncodetable: !!oncodetable, gymnastId });
    
    if (oncodetable && typeof oncodetable === 'function') {
      console.log('✅ Calling oncodetable function');
      try {
        oncodetable();
        console.log('✅ oncodetable executed successfully');
      } catch (error) {
        console.error('❌ Error executing oncodetable:', error);
        // Fallback al modal si hay error
        setVaultModalVisible(true);
      }
    } else {
      console.log('⚠️ No oncodetable prop or not a function, opening modal');
      setVaultModalVisible(true);
    }
  }, [oncodetable, gymnastId]);
  ...
  const handleVaultSelect = useCallback((vault: any, groupId: number, value: number, description: string) => {
    ...
    // Aquí puedes agregar lógica adicional para guardar la selección
    // Por ejemplo, actualizar la base de datos con el vault seleccionado
    try {
      // Lógica futura: guardar vault seleccionado en la base de datos
      console.log('✅ Vault selection processed');
    } catch (error) {
      console.error('❌ Error processing vault selection:', error);
    }
  }, [gymnastId]);
            ...
            imageGestureRef.current.longPressTimer = setTimeout(() => {
              Alert.alert(
                ...
                [
                  ...
                  { text: 'Eliminar', style: 'destructive', onPress: () => deleteImage(image.id) }
                ]
              );
            }, 800);
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        if (!currentlyManipulating) {
          ...
          if (touches && touches.length === 2) {
            ...
            updateImageTransform(currentSelectedId, { scale: newScale, rotation: newRotation });
          } else {
            const image = imagesRef.current.find(img => img.id === currentSelectedId);
            if (image) {
      ...
      <View {...panResponder.panHandlers} style={styles.drawingArea}>
        ...
        >
          <G>
            ...
            {/* Capa 3: Imagen de fondo del salto (SIEMPRE VISIBLE, nunca afectada por borrador) */}
            <SvgImage
              x={(width - width * 0.9) / 2}
              y={0}
              width={width * 0.9}
              height={height}
              href={require('../assets/images/Jump1.png')}
              opacity={0.6}
              preserveAspectRatio="xMidYMid meet"
            />
            
            {/* Capa 4: Imágenes del usuario (SIEMPRE ENCIMA, nunca afectadas por borrador) */}
            {images.map((img) => {
              ...
              return (
                <G key={img.id}>
                  ...
                  {isSelected && (
                    <Path
                      d={`M ${img.x} ${img.y} L ${img.x + img.width * img.scale} ${img.y} L ${img.x + img.width * img.scale} ${img.y + img.height * img.scale} L ${img.x} ${img.y + img.height * img.scale} Z`}
                      stroke="#007AFF"
                      strokeWidth={3}
                      fill="none"
