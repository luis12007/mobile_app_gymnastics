...
import { getGymnastById, updateGymnast, Gymnast } from '../lib/database';
...
interface WhiteboardProps {
  gymnastId: number; // Cambiado de tableId a gymnastId
  stickBonus?: boolean;
...
const MAX_PHOTOS_RENDERED = 20; // Límite de fotos renderizadas simultáneamente
...
const DrawingCanvasInner = ({ 
  ...
  const [photoItems, setPhotoItems] = useState<PhotoItem[]>([]); // objetos con transform
  const [activePhoto, setActivePhoto] = useState<string | null>(null);
  const photoItemsRef = useRef<PhotoItem[]>([]);
  const [imageMeta, setImageMeta] = useState<{ [uri: string]: { w: number; h: number } }>({});
  
  // Ref para cambios pendientes de fotos (debounce)
  const pendingPhotoUpdatesRef = useRef<Map<string, Partial<PhotoItem>>>(new Map());
  ...
  // Helper para registrar metadata de imagen
  const registerImageMeta = useCallback((uri: string, w: number, h: number) => {
    setImageMeta(prev => ({ ...prev, [uri]: { w, h } }));
  }, []);
  ...
  const loadSavedPaths = useCallback(async () => {
    try {
      ...
      const traces = await db.getAllAsync<{trace_data: string}>(
        'SELECT trace_data FROM whiteboard_traces WHERE gymnast_id = ? ORDER BY order_index',
        [gymnastId]
      );
      
      if (traces && traces.length > 0) {
        try {
          const savedPathsData: PathData[] = traces.map((t: {trace_data: string}) => JSON.parse(t.trace_data));
      ...
      console.error('Error in loadSavedPaths:', error);
    }
  }, [gymnastId]);
  ...
  const loadPhotos = useCallback(async () => {
    try {
      const db = await SQLite.openDatabaseAsync('gym.db');
      const result = await db.getAllAsync<{ image_uri: string }>(
        'SELECT image_uri FROM gymnast_images WHERE gymnast_id = ? ORDER BY order_index',
        [gymnastId]
      );
      const list = result.map((row: { image_uri: string }) => row.image_uri);
      setPhotos(list);
    } catch (error) {
  ...
  const loadPhotoItems = useCallback(async () => {
    try {
      console.log('📸 [loadPhotoItems] Loading photo items for gymnastId:', String(gymnastId));
      const db = await SQLite.openDatabaseAsync('gym.db');
      const items = await db.getAllAsync<GymnastImage>(
      ...
      // Filtrar y normalizar items, excluyendo URIs inválidos
      const normalized = items
        .filter((it: GymnastImage) => {
          if (!it.image_uri || typeof it.image_uri !== 'string' || it.image_uri.trim() === '') {
            console.warn('⚠️ [loadPhotoItems] Skipping item with invalid URI:', it);
            return false;
          }
          return true;
        })
        .map((it: GymnastImage, idx: number) => {
          const item = {
            uri: it.image_uri,
            x: it.position_x || 0,
            y: it.position_y || 0,
  ...
  const appendPhotoImportLog = useCallback((entry: Record<string, any>) => {
    const payload = { ts: new Date().toISOString(), platform: Platform.OS, gymnastId, screen: 'jump', ...entry };
  ...
  if (Platform.OS === 'android' || Platform.OS === 'ios') {
      ...
      const db = await SQLite.openDatabaseAsync('gym.db');
      await db.runAsync(
        `INSERT INTO gymnast_images (gymnast_id, image_uri, position_x, position_y, scale, rotation, order_index)
         VALUES (?, ?, ?, ?, ?, ?, (SELECT COALESCE(MAX(order_index), 0) + 1 FROM gymnast_images WHERE gymnast_id = ?))`,
        [gymnastId, pickedUri, centerX, centerY, 0.5, 0, gymnastId]
      );
      
      appendPhotoImportLog({ step: 'db_add_result', success: true, uri: pickedUri });
  ...
  }, [gymnastId, loadPhotos, loadPhotoItems]);
  ...
  const onUpdatePhotoTransform = useCallback(async (uri:string, data:{x?:number;y?:number;scale?:number;rotation?:number})=>{
    ...
    try {
      ...
      const result = await db.runAsync(
        `UPDATE gymnast_images SET position_x = ?, position_y = ?, scale = ?, rotation = ?
         WHERE gymnast_id = ? AND image_uri = ?`,
        [merged.x, merged.y, merged.scale, merged.rotation, gymnastId, uri]
      );
      ...
      if (result.changes === 0) {
        ...
        throw new Error('La operación de actualización no afectó ninguna fila');
      }
  ...
  const handleDeletePhoto = useCallback(async (uri:string) => {
    ...
    try {
      ...
      const db = await SQLite.openDatabaseAsync('gym.db');
      await db.runAsync(
        'DELETE FROM gymnast_images WHERE gymnast_id = ? AND image_uri = ?',
        [gymnastId, uri]
      );
  ...
  }, [gymnastId, photoItems, loadPhotos, loadPhotoItems]);
  ...
  const findPhotoAtPoint = useCallback((x: number, y: number): string | null => {
    for (let i = photoItems.length - 1; i >= 0; i--) {
      ...
      // Validar que la foto tiene datos válidos
      if (!photo || !photo.uri || typeof photo.x !== 'number' || typeof photo.y !== 'number') {
        continue;
      }
      
      const meta = imageMeta[photo.uri];
      if (!meta || !meta.w || !meta.h) continue;
  ...
  const savePaths = useCallback(async (newPathsData: PathData[]) => {
    ...
    try {
      ...
      // Eliminar trazos anteriores y agregar nuevos
      await db.runAsync('DELETE FROM whiteboard_traces WHERE gymnast_id = ?', [gymnastId]);
      
      // Insertar cada trazo con su orden
      for (let i = 0; i < limitedPaths.length; i++) {
        const traceData = JSON.stringify(limitedPaths[i]);
        const pathData = limitedPaths[i];
        await db.runAsync(
          `INSERT INTO whiteboard_traces (gymnast_id, trace_data, color, stroke_width, pen_type, order_index)
           VALUES (?, ?, ?, ?, ?, ?)`,
          [gymnastId, traceData, pathData.color, pathData.strokeWidth, pathData.penType || 'normal', i]
        );
      }
    ...
    return memo(({
      ...
      return (
        <Canvas style={[styles.canvas, { height: canvasHeight }]}>
          ...
          {Array.isArray(pathsData) && Children.toArray(normalPaths.map((pathInfo: any) => (
            <Path 
              key={`n-${pathInfo.idx}`} 
              path={pathInfo.path} 
          ...
          {/* Telestrator paths */}
          {Array.isArray(pathsData) && Children.toArray(telePaths.map((pathInfo: any) => (
            <Path 
              key={`t-${pathInfo.idx}`} 
              path={pathInfo.path} 
          ...
          {/* Highlighter paths */}
          {Array.isArray(pathsData) && Children.toArray(highlightPaths.map((pathInfo: any) => (
          ...
          {Array.isArray(visiblePhotos) && visiblePhotos.map((item:PhotoItem)=>(
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Percentage display */}
      <Text style={styles.percentageText}>{String(percentage !== null && percentage !== undefined ? percentage : '0')}%</Text>
...
const DrawingCanvas = (props: WhiteboardProps) => {
  const { gymnastId } = props || {};
  
  // Debug: mostrar información del gymnastId recibido
  console.log('🎯 WhiteboardScreen_jump - gymnastId recibido:', gymnastId, 'tipo:', typeof gymnastId);
  
  // Validación mejorada: aceptar números válidos mayores a 0
  if (!gymnastId || typeof gymnastId !== 'number' || Number.isNaN(gymnastId) || gymnastId <= 0) {
    console.error('❌ WhiteboardScreen_jump - gymnastId inválido:', {
      gymnastId,
      type: typeof gymnastId,
      isNaN: Number.isNaN(gymnastId),
      props: props
    });
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: '#ffe0e0', padding: 20 }}>
        <Text style={{ fontSize: 16, fontWeight: 'bold', marginBottom: 10 }}>⚠️ Whiteboard no disponible</Text>
        <Text style={{ fontSize: 12, color: '#666' }}>gymnastId: {String(gymnastId)}</Text>
        <Text style={{ fontSize: 12, color: '#666' }}>tipo: {typeof gymnastId}</Text>
      </View>
    );
  }

  console.log('✅ WhiteboardScreen_jump - Renderizando componente con gymnastId:', gymnastId);
  
  return <DrawingCanvasInner {...props} />;
};
